...
class MockLobby : public ILobby {
   public:
    ...
    MOCK_METHOD(void, msg_all, (std::string msg), (override));
...
class MockLobbyManager : public ILobbyManager {
   public:
    MOCK_METHOD(std::shared_ptr<ILobby>, create, (ISession & session),
                (override));

    MOCK_METHOD(std::shared_ptr<ILobby>, join,
                (const std::string& code, ISession& session), (override));
