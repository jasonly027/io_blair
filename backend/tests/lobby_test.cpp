...
TEST(LobbyUpdate, InvalidJson) {
...

  {
    ...
    EXPECT_CALL(*session, get_shared()).WillRepeatedly(ReturnPointee(&session));
...
TEST_F(LobbyUpdateCharacterSelectTest, Hover) {
  ...
  const string req_min64 = json::write(Hover<int64_t>{.hover = numeric_limits<int64_t>::min()});
  lobby_->update(*s1_, req_min64);

  const string req_max64 = json::write(Hover<int64_t>{.hover = numeric_limits<int64_t>::max()});
  lobby_->update(*s1_, req_max64);

  const string req_maxu64 = json::write(Hover<uint64_t>{.hover = numeric_limits<uint64_t>::max()});
  lobby_->update(*s1_, req_maxu64);

  const string req_decimal = json::write(Hover<double>{.hover = 1.0});
  lobby_->update(*s1_, req_decimal);

  const string req_unset
      = json::write(Hover<CharacterImpl>{.hover = static_cast<CharacterImpl>(Character::kUnset)});
  ...
  const string req_io = json::write(Hover<CharacterImpl>{.hover = req::CharacterSelect.hover.io});
  lobby_->update(*s1_, req_io);

  const string req_blair
      = json::write(Hover<CharacterImpl>{.hover = req::CharacterSelect.hover.blair});
...
TEST_F(LobbyUpdateCharacterSelectTest, ConfirmCharacter) {
  ...
  const string req_min64 = json::write(Confirm<int64_t>{.confirm = numeric_limits<int64_t>::min()});
  lobby_->update(*s1_, req_min64);

  const string req_max64 = json::write(Confirm<int64_t>{.confirm = numeric_limits<int64_t>::max()});
  lobby_->update(*s1_, req_max64);

  const string req_maxu64
      = json::write(Confirm<uint64_t>{.confirm = numeric_limits<uint64_t>::max()});
  lobby_->update(*s1_, req_maxu64);
  ...
  const string req_unset
      = json::write(Confirm<CharacterImpl>{.confirm = req::CharacterSelect.confirm.unset});
  const string req_io
