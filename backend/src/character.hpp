...
constexpr std::optional<Move> to_move(int64_t num) {
  ...
  switch (num) {
    case static_cast<MoveImpl>(kUp):    return kUp;
    case static_cast<MoveImpl>(kRight): return kRight;
    case static_cast<MoveImpl>(kDown):  return kDown;
    case static_cast<MoveImpl>(kLeft):  return kLeft;
    default:                            return std::nullopt;
  }
}
