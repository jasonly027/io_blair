...
using std::cout, std::cerr, std::string, std::string_view, std::size_t, std::shared_ptr;
using lock = std::lock_guard<std::mutex>;

WebSocketSession::WebSocketSession(net::io_context& ctx, tcp::socket socket, LobbyManager& manager)
...
void WebSocketSession::run() {
  ...
  ws_.async_accept([self = shared_from_this()](error_code ec) { self->on_accept(ec); });
}
...
auto WebSocketSession::state() const -> State {
  return state_;
}

void WebSocketSession::set_state(State state) {
  state_ = state;
}
...
auto WebSocketSession::fail(error_code ec, const char* what) -> Error {
  ...
  if (ec == websocket::condition::handshake_failed || ec == net::error::operation_aborted
...
void WebSocketSession::prepare_for_next_read() {
  ws_.async_read(buffer_, [self = shared_from_this()](error_code ec, size_t bytes) {
...
void WebSocketSession::on_read(error_code ec, size_t bytes [[maybe_unused]]) {
  ...
  net::post(read_strand_,
            [str = beast::buffers_to_string(buffer_.data()), self = shared_from_this()]() mutable {
...
void WebSocketSession::prepare_for_next_write() {
  ws_.async_write(
      ...
      net::bind_executor(write_strand_, [self = shared_from_this()](error_code ec, size_t bytes) {
...
void WebSocketSession::close() {
  ...
  ws_.async_close(websocket::close_code::normal, [self = shared_from_this()](error_code ec) {
    if (ec != net::error::operation_aborted) return log_err(ec, "ws async_close");
  });
}
