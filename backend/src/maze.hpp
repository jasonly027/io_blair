...
Direction constexpr get_opposite(Direction dir) {
    switch (dir) {
        case Direction::kUp:
...
class Cell {
   public:
    ...
    constexpr bool up() const {
        return bits_[kOffsetToIO + kOffsetToUp] ||
               bits_[kOffsetToBlair + kOffsetToUp];
    }

    // IO or Blair can see a path right
    constexpr bool right() const {
        return bits_[kOffsetToIO + kOffsetToRight] ||
               bits_[kOffsetToBlair + kOffsetToRight];
    }

    // IO or Blair can see a path down
    constexpr bool down() const {
        return bits_[kOffsetToIO + kOffsetToDown] ||
               bits_[kOffsetToBlair + kOffsetToDown];
    }

    // IO or Blair can see a path left
    constexpr bool left() const {
        return bits_[kOffsetToIO + kOffsetToLeft] ||
               bits_[kOffsetToBlair + kOffsetToLeft];
    }
    ...
    constexpr void set_path_io(Direction dir, bool value) {
        bits_[kOffsetToIO +
              static_cast<std::underlying_type_t<Direction>>(dir)] = value;
    }

    constexpr void set_path_blair(Direction dir, bool value) {
        bits_[kOffsetToBlair +
              static_cast<std::underlying_type_t<Direction>>(dir)] = value;
    }
    ...
    constexpr bool coin() const { return bits_[kOffsetToCoin]; }

    constexpr void set_coin(bool value) { bits_[kOffsetToCoin] = value; }
   ...
   private:
    constexpr int16_t serialize_impl(int offset_to_target_char,
                                     int offset_to_nontarget_char) const {
        std::bitset<9> res;

        for (int offset = 0; offset < kDirections; ++offset) {
            // Paths both can see
            res[offset] =
                bits_[offset + kOffsetToIO] && bits_[offset + kOffsetToBlair];
            // Paths only Blair can see
            res[offset + kOffsetToExclusivePath] =
                bits_[offset + offset_to_target_char] &&
                !bits_[offset + offset_to_nontarget_char];
        }
...
class Maze {
   public:
    ...
    constexpr static std::optional<position> get_neighbor(position pos,
                                                          Direction dir) {
        auto& [row, col] = pos;
        switch (dir) {
            case Direction::kUp:
    ...
    constexpr position start() const { return start_; }

    constexpr bool is_start(position pos) const { return pos == start_; }

    // Where to end in the maze
    constexpr position end() const { return end_; }

    constexpr bool is_end(position pos) const { return pos == end_; }

    // Will serialize with only io_path and coin per cell
    constexpr std::array<std::array<int16_t, kCols>, kRows> serialize_io()
        const {
    ...
    constexpr std::array<std::array<int16_t, kCols>, kRows> serialize_blair()
        const {
        std::array<std::array<int16_t, kCols>, kRows> res;
        for (int row = 0; row < kRows; ++row) {
            for (int col = 0; col < kCols; ++col) {
   ...
   private:
    ...
    constexpr void set_start(position pos) { start_ = pos; }

    constexpr void set_end(position pos) { end_ = pos; }
